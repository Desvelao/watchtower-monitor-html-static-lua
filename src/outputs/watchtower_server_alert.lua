...
local function alert_server(options, event, env, utils)
	local logger = utils.logger:get_logger({ name = "alert-server-connector:" .. event.data.id, level = "debug" })

	local method = "get"
	local alerts_url = options and options.url
	local data_id_prop_accessor = options and options.data_id_prop_accessor
	...
	if not data_id_prop_accessor then
		local err = "data_id_prop_name is not defined"
		logger.error(err)
		error(err)
	end

	local item_id = event:get(data_id_prop_accessor)

	local url_resolved = interpolate(alerts_url, { event = event, item_id = item_id })
	...
	if alerts ~= nil and alerts.items ~= nil then
		for i, alert in ipairs(alerts.items) do
			if alert.item.id == item_id then -- Ensure the alert is related to the item
				...
				if alert.enabled then
					...
					if alert.trigger_on_discount and event.data.discount then
						logger_alert.debug("Trigger alert by discount")
						should_generate_alert = true
					end

					if alert.trigger_on_available and event.data.available then
						logger_alert.debug("Trigger alert by available")
						should_generate_alert = true
					end

					if
						alert.trigger_on_price
						and utils.evaluate(string.format("return %s", alert.trigger_on_price), event.data)
					then
						...
						if alert.channels ~= nil then
							...
							for _, channel in ipairs(alert.channels) do
								...
								if destinations[channel.type] ~= nil then
									...
									local ok, err = destinations[channel.type](channel.options, {
										data = event.data,
